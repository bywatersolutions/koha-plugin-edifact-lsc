...
say "\nNot koha-plugin-edifact-lsc, exiting" && exit 0 unless $repo eq "koha-plugin-edifact-lsc";
...
foreach my $repo (@repos) {
    next if $repo eq 'koha-plugin-edifact-lsc';
    next if $repo eq 'koha-plugin-edifact-lsc-docs';
